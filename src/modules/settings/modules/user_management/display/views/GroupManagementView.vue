    ...
    >
      ...
      <div class="row justify-start items-center">
        <div class="row items-center cursor-pointer" @click="router.back()">
          ...
          <span class="q-ml-sm text-h6 text-bold"
            >Gestión Usuarios - Fichas</span
          >
        </div>
      </div>
      <!-- New and Search -->
...
onMounted(async () => {
  ...
  if (res.status === statusMessages.success) exampleGroupList.value = userManagementStore.classGroups;
  else customNotify({
    status: res.status,
    message: res.message
  });
